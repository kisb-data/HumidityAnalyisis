...
with colB:
    ...
    st.markdown('At this point, we can better understand our data, in monthly resolution. It is worthwhile to evaluate the data on a monthly basis, as the amount of data is not the same from month to month, and therefore distorts in annual terms. We can analyze on two charts, the absolute humidity on the left, and the difference of this value compared to the measurement outside on the right.')
